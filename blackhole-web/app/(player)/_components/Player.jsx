...
export default function Player({ id }) {
  ...
  const handleSeek = (e) => {
    ...
    // Check if the seekTime is a finite number
    if (isFinite(seekTime)) {
      audioRef.current.currentTime = seekTime;
      setCurrentTime(seekTime);
    } else {
      console.error("Invalid seekTime value:", seekTime);
    }
  };
  ...
  const playNextSong = async () => {
    setCurrentIndex((prevIndex) => {
      const isLastIndex = prevIndex + 1 >= next.songs.length;
      ...
      const nextSong = next.songs[nextIndex];

      if (nextSong && nextSong.id) {
        localStorage.setItem("last-played", nextSong.id);

        getSongsById(nextSong.id).then(async (get) => {
          const data = await get.json();
          const newSong = data.data[0];
          setData(newSong);

          const url =
            newSong?.downloadUrl[2]?.url ||
            newSong?.downloadUrl[1]?.url ||
            newSong?.downloadUrl[0]?.url;
      ...
      <div className="grid gap-6 w-full">
        <div className="sm:flex px-6 md:px-20 lg:px-32 grid gap-5 w-full">
            ...
            {data.length <= 0 ? (
              ...
              <div className="relative">
                <img
                  ...
                  className="sm:h-[150px] h-full bg-secondary/50 rounded-2xl sm:w-[200px] w-full object-cover"
                />
                <img
                  src={data.image[2].url}
          ...
          {data.length <= 0 ? (
            ...
            <div className="flex flex-col justify-between w-full">
              <div>
                <h1 className="text-xl font-bold md:max-w-lg max-w-[260px]">
                  {data.name}
                </h1>
              ...
              <div className="grid gap-2 w-full mt-5 sm:mt-0">
                ...
                <div className="flex items-center justify-between mt-2">
                  <div className="flex items-center gap-3 sm:mt-0">
                    ...
                    <Button
                      size="icon"
                      variant={playing ? "gooeyRight" : "gooeyLeft"}
